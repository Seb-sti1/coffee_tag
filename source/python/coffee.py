...
import pandas as pd
# Islice for list iteration not starting at 0
from itertools import islice
# Timer and sleeps
import time
# Threads for non-blocking timer callbacks
import threading
...
# 
# 125 kHz reader : https://www.gotronic.fr/art-lecteur-rfid-grove-125-khz-113020002-19038.htm
...
class BadgeEntry:
    def __init__(self, methodToRun, root):
        ...
        # Period in seconds
        self.delay = 0.1
        # Delay before discarding last badge info
        self.timeout = 5
...
class Finder:
    ...
    def manual_entry(self):
        ...
        new_root.protocol("WM_DELETE_WINDOW", lambda: [on_closing(), new_root.destroy()])
        global current_windows
        current_windows += 1
        new_root.transient(self.main) # set to be on top of the main window
        new_root.grab_set() # hijack all commands from the master (clicks on the main window are ignored)
        ...
        new_txt_lbl = tk.Label(new_root, text="What is your name ?", font='Helvetica 14 bold italic', fg="white", bg='#754c24')
        ...
        new_bt_label = tk.Button(new_root, text="Find me !", font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', command=lambda: self.find_entry(entry, new_root, Identification.NAME))
    ...
    def confirm(self, entry, root, index):
        ...
        if coffee > 9:
            ...
            new_root['bg']='#754c24'
            new_root.resizable(height=False,width=False)
            warning_lbl = tk.Label(new_root, text="Wow !", font='Helvetica 22 bold', fg="white", bg='#754c24')
            warning_lbl.pack(side="top", pady=10, fill='x')
            amount_lbl = tk.Label(new_root, text="Do you confirm " + str(coffee) + " coffees ?", wraplength=220, justify="center", font='Helvetica 15',  fg='#c9a589', bg='#754c24')
            amount_lbl.pack(side="top")
            # Buttons
            yes_bt_lbl = tk.Button(new_root, text="Yes", font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', height=1, width=10, command=lambda: [self.update_csv(entry, root, index), new_root.destroy(), on_closing(), self.summation(index)])
            yes_bt_lbl.pack(side="top", pady=10)
            oops_bt_lbl = tk.Button(new_root, text="Oops", font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', height=1, width=10, command=lambda: [new_root.destroy(), on_closing()])
    ...
    def summation(self, index):
        ...
        new_root.protocol("WM_DELETE_WINDOW", lambda: [on_closing(), new_root.destroy()])
        current_windows += 1
        new_root.transient(self.main) # set to be on top of the previous window
        new_root.grab_set()
        ...
        current_windows -= 1
        user = ""
        if not self.my_csv_data[index][2] == "" and not pd.isna(self.my_csv_data[index][2]):
            user = str(self.my_csv_data[index][2])
        else:
            user = str(self.my_csv_data[index][0])
        welcome_lbl = tk.Label(new_root, text="Thank you " + str(user) + ",", wraplength=280, justify="center", font='Helvetica 22 bold', fg="white", bg='#754c24')
        welcome_lbl.pack(side="top", pady=10, fill='x')
        txt_lbl = tk.Label(new_root, text="Your account is now", font='Helvetica 15',  fg='#c9a589', bg='#754c24')
        txt_lbl.pack(side="top")
        amount = -float(self.my_csv_data[index][5])
        amount_lbl = tk.Label(new_root, text=str(round(amount, 2)) + " €", font='Helvetica 22 bold', fg="white", bg='#754c24')
        amount_lbl.pack(side="top", pady=10, fill='x')
        # automatic close
        closing_lbl = tk.Label(new_root, font='Helvetica 12 bold italic', fg="white", bg='#754c24')
        closing_lbl.pack(side="top", fill='x', pady=30)
        for i in range (5, -1, -1):
            closing_lbl.config(text="Closing window in " + str(i) + " seconds...")
            new_root.update()
            time.sleep(1)
        new_root.destroy()
        current_windows -= 1
    ...
    def update_csv(self, entry, root, index):
        coffee = int(entry.get())
        price = coffee*0.2
        #print("It will cost %.2f € to %s" % (round(price, 2), self.my_csv_data[index][0]))
        global current_windows
        current_windows -= 1
        root.destroy()
        # Note: in csv file, a debt is positive and an advance negative
        new_debt = float(self.my_csv_data[index][5])+price
        #print("New debt is %.2f" % -round(new_debt, 2))
    ...
    def found(self, index):
        ...
        if not self.my_csv_data[index][2] == "" and not pd.isna(self.my_csv_data[index][2]) :
            ...
            user = str(self.my_csv_data[index][0])
        welcome_lbl = tk.Label(new_root, text="Hello " + str(user) + " !", font='Helvetica 22 bold', fg="white", bg='#754c24')
        welcome_lbl.pack(side="top", pady=10, fill='x')
        txt_lbl = tk.Label(new_root, text="Your account is currently", font='Helvetica 15',  fg='#c9a589', bg='#754c24')
        txt_lbl.pack(side="top")
        amount = -float(self.my_csv_data[index][5])
        amount_lbl = tk.Label(new_root, text=str(round(amount, 2)) + " €", font='Helvetica 22 bold', fg="white", bg='#754c24')
        amount_lbl.pack(side="top", pady=10, fill='x')
        
        coffee_lbl = tk.Label(new_root, text="How many coffees will you take ?", font='Helvetica 15',  fg='#c9a589', bg='#754c24')
        coffee_lbl.pack(side="top")
        #Create entry and buttons to set the amount of coffee
        coffees = 0
        entry = tk.Entry(new_root, textvariable=coffees, width=3, font='Helvetica 15 bold')
        ...
        incr_bt_lbl = tk.Button(new_root, text="►", font='Helvetica 25', fg='#5b3719', bg='#c9a589', height=1, width=1, command=lambda: self.update_coffee(1, entry))
        incr_bt_lbl.place(x=230, y=185)
        incr_bt_lbl = tk.Button(new_root, text="◄", font='Helvetica 25', fg='#5b3719', bg='#c9a589', height=1, width=1, command=lambda: self.update_coffee(-1, entry))
        incr_bt_lbl.place(x=90, y=185)
        # A button to validate number of coffees to count
        bt_lbl = tk.Button(new_root, text="OK", font='Helvetica 14 bold', fg='#5b3719', bg='#c9a589', height=1, width=2, command=lambda: self.confirm(entry, new_root, index))
    ...
    def register(self, root, name_entry, surname_entry, mail_entry, nickname_entry):
        ...
        if name == "" or surname == "" or mail == "":
            ...
            pop_root.protocol("WM_DELETE_WINDOW", lambda: [on_closing(), pop_root.destroy()])
            current_windows += 1
            ...
            pop_root['bg']='#754c24'
            sorry_lbl = tk.Label(pop_root, text="Well...", font='Helvetica 22 bold', fg="white", bg='#754c24')
            sorry_lbl.pack(side="top", pady=10, fill='x')
            warn_lbl = tk.Label(pop_root, text="You must provide at least your name, surname and mail", wraplength=240, justify="center", font='Helvetica 12 bold italic', fg='#c9a589', bg='#754c24')
            warn_lbl.pack(side="top", pady=10, fill='x')
            bt_lbl = tk.Button(pop_root, text="OK", font='Helvetica 16 bold', fg='#5b3719', bg='#c9a589', height=2, width=4, command=lambda: self.erase(pop_root))
            ...
            for i in range(0, len(self.my_csv_data)):
                # Look for the user string in Surname, Name, Nickname, or badge ID /!\ For NaN values, csv data is considered as a float, so cast in string if empty
            ...
            new_root['bg']='#754c24'        
            if error == Error.MAIL:
                well_lbl = tk.Label(new_root, text="Well...", font='Helvetica 22 bold', fg="white", bg='#754c24')
                well_lbl.pack(side="top", pady=10, fill='x')
                warn_lbl = tk.Label(new_root, text="It seems your mail is already linked to the account", wraplength=240, justify="center", font='Helvetica 12 bold italic', fg='#c9a589', bg='#754c24')
                warn_lbl.pack(side="top", pady=10, fill='x')
                name_lbl = tk.Label(new_root, text=str(self.my_csv_data[user_index][0]) + " " + str(self.my_csv_data[user_index][1]), wraplength=240, justify="center", font='Helvetica 12 bold', fg='white', bg='#754c24')
                name_lbl.pack(side="top", pady=10, fill='x')
                bt_lbl = tk.Button(new_root, text="OK", font='Helvetica 16 bold', fg='#5b3719', bg='#c9a589', height=2, width=4, command=lambda: self.erase(new_root))
                bt_lbl.pack(side="top", pady=20)
            elif error == Error.NAME:
                well_lbl = tk.Label(new_root, text="Well...", font='Helvetica 22 bold', fg="white", bg='#754c24')
                well_lbl.pack(side="top", pady=10, fill='x')
                name_lbl = tk.Label(new_root, text= name + " " + surname, wraplength=240, justify="center", font='Helvetica 12 bold', fg='white', bg='#754c24')
                name_lbl.pack(side="top", pady=10, fill='x')
                warn_lbl = tk.Label(new_root, text = "is already in the database", wraplength=240, justify="center", font='Helvetica 12 bold italic', fg='#c9a589', bg='#754c24')
                ...
                root.destroy()
                current_windows -= 1
                welcome_lbl = tk.Label(new_root, font='Helvetica 22 bold', fg="white", bg='#754c24')
                # Display name and adjust windows-wrapped label depending on the length (wraplength)
                if len(nickname) == 0:
                    welcome_lbl.config(text = "Welcome " + surname + " !", wraplength=280, justify="center")
                else:
                    welcome_lbl.config(text = "Welcome " + nickname + " !", wraplength=280, justify="center")
                welcome_lbl.pack(side="top", pady=10, fill='x')
                profile_lbl = tk.Label(new_root, text="Your profile is now created", font='Helvetica 12 bold italic', fg='#c9a589', bg='#754c24')
    ...
    def link(self, root, combo_box, badge_string):
        ...
        welcome_lbl = tk.Label(new_root, text="Hello " + str(user) + ",", font='Helvetica 22 bold', fg="white", bg='#754c24')
        welcome_lbl.pack(side="top", pady=10, fill='x')
        amount_lbl = tk.Label(new_root, text="Your badge has been successfully linked to yout account", font='Helvetica 14',  fg='#c9a589', bg='#754c24', wraplength=280, justify="center")
        amount_lbl.pack(side="top")
    ...
    def synchronize(self, root, badge_string):
        ...
        new_root['bg']='#754c24'
        new_root.resizable(height=False,width=False)
        txt_lbl = tk.Label(new_root, text="Select a account to synchronize with", font='Helvetica 16 bold', fg='#c9a589', bg='#754c24')
        ...
        # Button
        sync_bt_lbl = tk.Button(new_root, text="Link", font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', height=1, width=10, command=lambda: self.link(new_root, combo_box, badge_string))
    ...
    def add_user(self, root):
        ...
        new_root['bg']='#754c24'
        new_root.resizable(height=False,width=False)
        txt_lbl = tk.Label(new_root, text="Enter your data", font='Helvetica 16 bold', fg='#c9a589', bg='#754c24')
        txt_lbl.pack(side="top", pady=10, fill='x')
        ...
        # Name
        name_lbl = tk.Label(new_root, text="Name", font='Helvetica 12 bold italic', fg="white", bg='#754c24')
        #name_lbl.pack(side="top", pady=10, fill='x')
        name_lbl.place(x=40, y=50)
        name_entry = tk.Entry(new_root, textvariable=user_name, width=20, font='Helvetica 12')
        name_entry.focus_set()
        #name_entry.pack(side="top")
        name_entry.place(x=40, y=80)
        # Surname
        surname_lbl = tk.Label(new_root, text="Surname", font='Helvetica 12 bold italic', fg="white", bg='#754c24')
        #surname_lbl.pack(side="top", pady=10, fill='x')
        surname_lbl.place(x=40, y=110)
        surname_entry = tk.Entry(new_root, textvariable=user_surname, width=20, font='Helvetica 12')
        #surname_entry.pack(side="top")
        surname_entry.place(x=40, y=140)
        # Nickname
        nickname_lbl = tk.Label(new_root, text="Nickname (optional)", font='Helvetica 12 bold italic', fg="white", bg='#754c24')
        #nickname_lbl.pack(side="top", pady=10, fill='x')
        nickname_lbl.place(x=40, y=170)
        nickname_entry = tk.Entry(new_root, textvariable=user_nickname, width=20, font='Helvetica 12')
        #nickname_entry.pack(side="top")
        nickname_entry.place(x=40, y=200)
        # Mail
        mail_lbl = tk.Label(new_root, text="E-mail address", font='Helvetica 12 bold italic', fg="white", bg='#754c24')
        #mail_lbl.pack(side="top", pady=10, fill='x')
        mail_lbl.place(x=250, y=50)
        mail_entry = tk.Entry(new_root, textvariable=user_mail, width=35, font='Helvetica 12')
        #mail_entry.pack(side="top")
        mail_entry.place(x=250, y=80)
        # Badge
        badge_txt_lbl = tk.Label(new_root, text="Swipe your ENSTA badge if you have one to synchronize it with your profile",  wraplength=320, font='Helvetica 12 bold italic', fg='#c9a589', bg='#754c24')
        ...
        badge_lbl = BadgeLabel(new_root, width=35, font='Helvetica 12 bold', fg="white", bg='#754c24', borderwidth=1, highlightthickness=1)
        #badge_lbl.pack(side="top")
        badge_lbl.place(x=250, y=200)
        badge_lbl.start(100)#100ms update on label
        # Button
        add_bt_lbl = tk.Button(new_root, text="OK", font='Helvetica 16 bold', fg='#5b3719', bg='#c9a589', height=2, width=4, command=lambda: self.register(new_root, name_entry, surname_entry, mail_entry, nickname_entry))
        #add_bt_lbl.pack(side="top", pady=20)
    ...
    def find_me(self, user_string, root, identification):
        ...
        if user_string == "":
            ...
            sorry_lbl = tk.Label(pop_root, text="Well...", font='Helvetica 22 bold', fg="white", bg='#754c24')
            sorry_lbl.pack(side="top", pady=10, fill='x')
            warn_lbl = tk.Label(pop_root, text="You must provide at least your name, surname or nickname", wraplength=240, justify="center", font='Helvetica 12 bold italic', fg='#c9a589', bg='#754c24')
            warn_lbl.pack(side="top", pady=10, fill='x')
            bt_lbl = tk.Button(pop_root, text="OK", font='Helvetica 16 bold', fg='#5b3719', bg='#c9a589', height=2, width=4, command=lambda: self.erase(pop_root))
            bt_lbl.pack(side="top", pady=20)
        else:
            ...
            for i in range(0, len(self.my_csv_data)):
                # Look for the user string in Surname, Name, Nickname, or badge ID /!\ For NaN values, csv data is considered as a float, so cast in string if empty
                if ((user_string.lower() == str(self.my_csv_data[i][0]).lower()) or
                    (user_string.lower() == str(self.my_csv_data[i][1]).lower()) or
                    (user_string.lower() == str(self.my_csv_data[i][2]).lower()) or
            ...
            if found == 0:
                ...
                new_root['bg']='#754c24'
                new_root.resizable(height=False,width=False)
                sorry_lbl = tk.Label(new_root, text="Sorry !", font='Helvetica 22 bold', fg="white", bg='#754c24')
                sorry_lbl.pack(side="top", pady=10, fill='x')
                txt_lbl = tk.Label(new_root, text="I could not find you", font='Helvetica 12 bold italic', fg='#c9a589', bg='#754c24')
                txt_lbl.pack(side="top", fill='x')
                if identification == Identification.NAME:
                    new_root.geometry('200x200')
                    bis_bt_lbl = tk.Button(new_root, text="Try again", font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', height=1, width=10, command=lambda: self.erase_and_manual_entry(new_root))
                    bis_bt_lbl.pack(side="top", pady=10)
                    add_bt_lbl = tk.Button(new_root, text="Add me", font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', height=1, width=10, command=lambda: self.add_user(new_root))
                    add_bt_lbl.pack(side="top")
                elif identification == Identification.BADGE:
                    new_root.geometry('270x230')
                    add_bt_lbl = tk.Button(new_root, text="Add me", font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', height=1, width=8, command=lambda: self.add_user(new_root))
                    add_bt_lbl.pack(side="top", pady=10)
                    exist_txt_lbl = tk.Label(new_root, text="Former user with new badge ?", font='Helvetica 12 bold italic', fg='white', bg='#754c24')
                    exist_txt_lbl.pack(side="top", pady=10, fill='x')
                    bis_bt_lbl = tk.Button(new_root, text="Synchronize", font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', height=1, width=14, command=lambda: self.synchronize(new_root, user_string))
            ...
            #    self.found(users_index[0])
                ...
                #print("I found", found, user_string)
                new_root = tk.Toplevel(self.main)
                new_root.protocol("WM_DELETE_WINDOW", lambda: [on_closing(), new_root.destroy()])
                current_windows += 1
                new_root.transient(self.main) # set to be on top of the main window
                ...
                geometry = "300x" + str(210+40*len(users_index)) + "+247+120"
                new_root.geometry(geometry)
                new_root.title('Precise')
                new_root['bg']='#754c24'
                new_root.resizable(height=False,width=False)    
                new_lbl = tk.Label(new_root, text = "Hello !", font='Helvetica 22 bold', fg="white", bg='#754c24')
#                 if found == 1:
...
#                     new_lbl.config(text = "Sorry !")
                ...
                if identification == Identification.NAME:
                    found_lbl = tk.Label(new_root, text=("I found " + str(found) + " " + user_string), font='Helvetica 14 bold', fg='#c9a589', bg='#754c24')
                    found_lbl.pack(side="top", fill='x')
                txt_lbl = tk.Label(new_root, text="Are you... ?", font='Helvetica 12 bold italic', fg='#c9a589', bg='#754c24')
                txt_lbl.pack(side="top", pady=10, fill='x')
                for i in range(0, len(users_index)):
                    ...
                    # Using the i=i trick causes the function to store the current value of i at the time lambda is defined, instead of waiting to look up the value of i later.
                    bt_lbl = tk.Button(new_root, text=self.my_csv_data[users_index[i]][0] + " " + self.my_csv_data[users_index[i]][1], font='Helvetica 12 bold', fg='#5b3719', bg='#c9a589', height=1, width=18, command=lambda i=i: self.erase_and_found(new_root, users_index[i]))
                    bt_lbl.pack(side="top")
                not_you_lbl = tk.Label(new_root, text="It's not you ?", font='Helvetica 12 bold', fg='white', bg='#754c24')
...
finder = Finder('/home/pi/Documents/coffee/users.csv', root)

## Labels
# Text labels
txt_lbl1 = tk.Label(root, text="You should take a break...", font='Helvetica 22 bold', fg="white", bg='#754c24')

# Place the text label on the window, 10 pixels from the top, and fill the window on x
txt_lbl1.pack(side="top", pady=10, fill='x')
txt_lbl2 = tk.Label(root, text="Badge for a coffee (0.20€)", font='Helvetica 16 bold', fg='#c9a589', bg='#754c24')
txt_lbl2.pack(side="top", fill='x')
# Button label to manually check identity
bt_lbl = tk.Button(root, text="Can't read my badge ?", font='Helvetica 15 bold', fg='#5b3719', bg='#c9a589', height=2, width=24, command=lambda: finder.manual_entry())
